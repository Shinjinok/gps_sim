...
def satpos(eph, g):
    ...
    #xpk = rk*cuk
    #ypk = rk*suk
    xypk = np.array([rk*cuk,rk*suk,0])
    aaa = np.array([[cuk,-suk,0],[suk,cuk,0],[0,0,0]])
    xypkdot = aaa.dot([rkdot,rk*ukdot,0])
    ...
    aa = np.array([[cok, -cik*sok, 0],
                   [sok ,cik*cok, 0],
                   [0, sik, 0]])
    #bb = np.array([xpk, ypk,0])
    pos = aa.dot(xypk)
    ...
    #aa = np.array([[cok,-sok,0],
     #              [sok,cok,0],
    #               [0,sik,0]])
    #bb = np.array([xpkdot,ypkdot,0])

    cc = np.array([[0, -eph.omgkdot,0],
                   [eph.omgkdot,0,0],
                   [0,0,0]])
    dd = np.array([pos[2]*sok,-pos[2]*cok,xypk[1]*cik])
    vel = aa.dot(xypkdot) + cc.dot(pos) + dd*ikdot
...
def ltcmat(llh):
   # t = np.zeros((3,3))
    ...
    t = np.array([[-slat*clon,-slat*slon,clat],
                   [-slon,clon,0.0],
...
if __name__ == "__main__":
    ...
    for iumd in range(numd):
        ...
        a = np.zeros(CHAN*iq_buff_size)
        b = np.zeros(CHAN*iq_buff_size)
        c = np.zeros(CHAN*iq_buff_size)
        d = np.zeros(CHAN*iq_buff_size)
        s = np.zeros(CHAN*iq_buff_size)  
        ...
        for isamp in range(iq_buff_size):
            i_acc = sum(ip[isamp:isamp+CHAN])
            q_acc = sum(qp[isamp:isamp+CHAN])
            # Scaled by 2^7
        ...
        grx = incGpsTime(grx, 0.1)
        # Update time counter
        print("\rTime into run = {:4.1f}".format(subGpsTime(grx, g0)),start2-start,start3-start2,
                            time.clock_gettime(time.CLOCK_REALTIME)-start3)
